...
Before any SPEC-004 or later revision advances from `draft` to `architecture_reviewed`, SPEC-000 revision 1 must enter its human-decided amendment state and a human-merged SPEC-000 revision 2 must define a digest-pinned `AuthorityVocabularyRegistry`. The constitutional rule remains deny-by-default. An entry binds action, resource, owner specification, actor-specific typed decision predicates, a structured maximum effect, exact dependency requirements, grant operation, and exhaustive allow/deny fixtures. SPEC-000 binds the registry and `governance/authority-vocabulary.schema.json` independently by canonical path, version, and exact digest; changing either binding therefore requires a new human-merged SPEC-000 revision under the ordinary amendment lifecycle. Authority changes are intentionally rare and are not an extension point for runtime policy, models, or ordinary command-family PRs.

The standalone authority schema is dormant machinery while SPEC-000 remains at revision 1. Schema presence, schema validity, an inventory record, or an offline conformance result grants no authority. Revision 2 must supply `Authority vocabulary schema`, `Authority vocabulary schema digest`, and `Authority vocabulary schema version` together with the corresponding three registry fields; this atomic six-key set is the only direct binding. A partial binding, a registry that attempts to select the schema used to parse itself, or a detached schema, registry, fixture, or receipt fails closed. The schema's major version 2 deliberately rejects the development-only version-1 registry shape rather than preserving an unactivated compatibility reader. The dependency-light runtime loader accepts only the exact schema bytes whose digest is code-pinned in the authority validator; CI separately meta-validates those same bytes as Draft 2020-12 and validates both document variants. The schema is also the third component of authority evaluator bundle `1.0.0`; changing its exact bytes changes its code pin, direct SPEC-000 binding, and future receipt bundle identity.

Each entry groups `actor_bindings` by actor class because different actor classes can require different predicates for the same action and resource. Predicates are closed, typed clauses over boolean, portable unsigned-integer, bounded string, SHA-256-digest, and UTC-datetime values. `present` supports every nonboolean type, requires the key, and validates its value against the declared type, including bounds or format; booleans therefore require an explicit `equals true` or `equals false`. `equals` requires exact typed equality, and `less_than_or_equal` is limited to portable nonnegative integers. Missing values, false values where true is required, empty or edge-whitespace strings, control characters, type or format mismatches, floating-point values, and duplicate predicate keys deny. `max_effect` is a ceiling rather than an exact requested effect: effect codes must be equal and both requested target count and state-transition count must be less than or equal to the declared components. Different effect codes are incomparable, and narrowing one component cannot compensate for widening the other.

Dependency requirements are structured `{spec_id, revision, minimum_runtime_stage}` records. Revisions match exactly, not as a greater-than-or-equal floor, and the declared minimum is limited to `implemented`, `verified`, or `operational`; every current profile conservatively requires `operational`, while `accepted` and terminal stages are never runtime-ready. M0B exercises those comparisons only over tuples marked `synthetic_offline`. It does not yet prove that a record is current or promoted. Schema-2 `identity_bindings` accept only canonical lowercase `synthetic:<token>` fixture labels, and separation-of-duty validation is structural offline evidence rather than provider identity resolution. The public catalog and fixtures contain no real principal, assignment, credential, grant, or protected-default evidence. A future runtime boundary must obtain authenticated actor class, the exact current promoted specification snapshot, active assignment and fence, and verified capability grant from trusted resolvers owned by their respective specifications rather than from caller JSON; that boundary must deny accepted, amended, superseded, retired, unpromoted, stale, missing, extra, and higher-revision records.
...
Registry validity is design evidence, not proof that the policy oracle implements it. Before SPEC-000 revision 2 may become `implemented`, the deterministic governance validator must execute every registry fixture against the exact `governance/constitution.yaml` digest and emit `governance/generated/authority-vocabulary-conformance.json`. The receipt binds constitution, registry, fixture manifest, and the complete semantic evaluator bundle: the authority schema, constitution decision oracle, and closed authority-contract validator. The bundle manifest has sorted path-and-digest components and its own domain-versioned aggregate digest, so unrelated lifecycle orchestration changes do not churn constitutional evidence and none of the three semantic components can change unnoticed. Receipt verification also requires the bound component bytes to equal the canonical, non-symlink source files backing the executing evaluator; copied fixture roots pass only when their evaluator bytes are exact.

Receipt schema `3.0.0` is deliberately scoped to `offline_class_policy_conformance` and declares `runtime_authority: none`; individual policy decisions likewise disclose `scope: offline_class_policy_decision` and `runtime_authority: none`. Its synthetic cases record a class-policy allow for every permitted actor binding; record denials for every mandatory-predicate omission, wrong actor, widened effect, dependency mismatch, and wrong grant; prove unknown pairs deny; and contain no skipped case. They do not authenticate a caller, prove a promoted runtime snapshot, mint a capability, or authorize an effect. The same offline gate requires the exact supported schema version, an `effective` policy bound through `effective_commit: "$SELF"`, reviewed actor automation descriptors, synthetic-only identity fixtures, the carried-forward protected-surface minimum including every evaluator component, unchanged emergency and amendment fail-safe invariants, and the offline-only allow reason code. It then structurally closes the complete policy allow-rule set: every mutating or protected allow rule must resolve to one catalog profile and may not add an actor, omit or replace a mandatory predicate, widen an effect, weaken an exact dependency requirement, change the grant operation, or hide an alternate permissive condition. The exact default-deny oracle admits no explicit deny rules; denied behavior must come from the closed default, an unsatisfied predicate, or a registry-boundary failure, so a selectively targeted denial cannot evade the finite fixtures while disabling a reviewed action. The only noncatalog allow rules are separately validated nonmutating `read/*` rules, which the runtime boundary rejects for event append. A retained legacy `publish_draft` rule or any other noncatalog state-changing allow fails even when no finite fixture happens to satisfy its condition. CI recomputes the receipt and rejects a stale, partial, or hand-edited result. Draft, architecture-reviewed, and accepted revisions forbid a prospective receipt; implemented, verified, and operational revisions require it. An amended, superseded, or retired revision may retain its last valid receipt as historical evidence but gains no active authority from it; a never-implemented terminal revision need not create one, and any retained receipt must still validate exactly. The lifecycle stage gate therefore prevents any downstream specification from becoming `implemented` while the registry is merely declarative or the effective oracle still denies or over-allows its required pair. `validate_authority_contract.py` owns these schema, registry, fixture, semantic, closure, bundle, and receipt rules; `validate_spec_lifecycle.py` owns the temporal stage gate and consumes that contract without redefining it.
