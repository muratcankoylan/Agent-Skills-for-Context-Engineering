...
The program is not a backlog of features. Each specification fixes an interface, authority boundary, state transition, or quality gate. Implementation details may change inside those contracts. Merging this roadmap publishes draft contracts; it does not accept them. Acceptance requires an explicit status transition in a human-merged pull request. A change to an accepted contract requires a specification amendment and a human merge.

## Delivery rules

1. One implementation pull request should normally implement one specification or one explicitly named vertical slice of it.
...
3. Human merge is the only production-promotion event. Approval without merge is review state, not deployment state.
...
8. New automation first runs in observation mode, then shadow mode, then proposal mode. Autonomous publication or merge is not in scope.
...
```text
draft -> architecture_reviewed -> accepted -> implementing
...
- `implementing`: an implementation pull request is open.
- `implemented`: code and migrations exist, but operational acceptance may remain.
- `verified`: acceptance tests and shadow evidence pass.
- `operational`: production projection is active and documented.
- `amended`, `superseded`, `retired`: immutable historical status plus a pointer to the replacing decision.
...
| 6 | Optional training research | 026 | Weight-level learning begins only after artifact-level learning has a stable, auditable dataset |
...
flowchart TD
    ...
    S003 --> S005["SPEC-005 Work Orders, Scheduling, Leases, and Recovery"]
    S004 --> S005
    S004 --> S006["SPEC-006 Command Bus and Human Feedback Ledger"]
    S005 --> S006
    S000 --> S007["SPEC-007 GitHub App and Pull-Request Lifecycle"]
    S004 --> S007
    S005 --> S007
    S006 --> S007
    S004 --> S008["SPEC-008 Status, Observability, and Cost Accounting"]
    S005 --> S008
    S006 --> S008

    S003 --> S009["SPEC-009 Source Registry and Feed Policy"]
    S009 --> S010["SPEC-010 Retrieval and Immutable Evidence Capture"]
    S010 --> S011["SPEC-011 Evidence Graph, Claims, Mechanisms, and Decisions"]
    ...
    S005 --> S013
    S012 --> S013
    S005 --> S014["SPEC-014 Execution Environment, Executor Protocol, and Hermes Integration"]
    S013 --> S014
    S002 --> S015["SPEC-015 Notification Outbox, Review Packets, and Content Drafts"]
    S006 --> S015
    S007 --> S015

    S011 --> S016["SPEC-016 Evaluation Registry, Rubric Epochs, and Fixtures"]
    S012 --> S016
    ...
    S017 --> S018
    S007 --> S019["SPEC-019 Pull-Request Quality Gate, Promotion, and Release"]
    S018 --> S019

    S014 --> S020["SPEC-020 Meta-Harness Experiment Laboratory"]
    S016 --> S020
    S018 --> S020
    S019 --> S020
    S020 --> S021["SPEC-021 Adaptive Harness Routing and Cross-Model Transfer"]
    S014 --> S022["SPEC-022 Collaborative Evolution and Community Contribution Packets"]
    S021 --> S022

    S007 --> S023["SPEC-023 Open-Source Governance and Project Growth"]
    S015 --> S023
    S019 --> S023
    S022 --> S023
    ...
    S015 --> S024
    S017 --> S024
    S024 --> S025["SPEC-025 Deployment, Recovery, and Durable Workflow Migration"]
    S022 --> S026["SPEC-026 Future Training and Reinforcement-Learning Laboratory"]
    S025 --> S026
```

The graph expresses minimum prerequisites, not a requirement to serialize every file change. Implementation may overlap within a wave only when each pull request can pass the prior wave's contracts independently.
...
| [SPEC-019](SPEC-019-promotion-release.md) | PR quality gate and release | Candidate observation, exact-SHA attestation, promotion record, and deployment projection |
...
| Spec | Contract | Primary deliverable |
|---|---|---|
| [SPEC-026](SPEC-026-training-rl-lab.md) | Future training and RL lab | Governed offline dataset and go/no-go protocol for weight updates |
...
| 014 + 016 + 018 + 019 -> 020 -> 021 -> 022 | Adds isolated recursive improvement only after environment, coverage, archive, and promotion contracts exist | Shadow runs beat equal-budget controls on calibrated holdouts without touching evaluators or production routing |
...
9. After a human merge, consume the merge webhook and project the accepted revision as production.
10. Observe one full operating cycle before marking the specification operational.
