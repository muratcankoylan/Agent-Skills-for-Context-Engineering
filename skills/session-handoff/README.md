...
Persists task progress across AI sessions via structured JSON records and an enforced state machine. When an AI session expires mid-task, the next session picks up exactly where the last left off — with full progress summary, next steps, blockers, and recent decisions intact on disk.
