...
```
python3 -m unittest discover -s researcher/scripts/tests -p "test_*.py" # researcher script regression tests
...
14. **Run `python3 -m unittest discover -s researcher/scripts/tests -p "test_*.py"`, `validate_platform_compat.py --require-reference-validator`, `validate_repo.py --strict`, `skill_health.py --strict --no-history`, `check_activation_cases.py`, and `run_benchmarks.py`** before committing skill changes
