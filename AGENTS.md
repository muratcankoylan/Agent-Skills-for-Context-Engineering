...
- The current prepared release version is 2.4.0 across `.claude-plugin/marketplace.json`, `.plugin/plugin.json`, and root `SKILL.md`. There are 16 skills (latent-briefing covers KV cache sharing between agents; self-improvement-loops covers RSI, meta-harness search, and evolutionary scaffold optimization).
...
- Benchmark execution uses native Codex CLI through Headroom at `researcher/benchmarks/codex-runner/`. It supports `--concurrency N`, `--no-resume`, per-run progress logging, format-failure retry, and worst-case retry-aware run caps; default behavior resumes by skipping completed result files. Result artifacts under `researcher/benchmarks/{router,effectiveness}/results/` and history JSONLs (`router-history.jsonl`, `effectiveness-history.jsonl`) are gitignored.
...
- Benchmarks use `/home/hermesadmin/.codex` through localhost Headroom and ChatGPT/Codex OAuth. The runner must explicitly set `HOME=/home/hermesadmin` and `CODEX_HOME=/home/hermesadmin/.codex` because isolated Hermes shells replace `HOME`. Conditions use fresh workspaces and only curated `.codex/skills/` directories. A hard `--max-runs` gate is required before any model call.
...
- A skill is a multi-surface artifact. Changing the frontmatter `description` is not enough; the SKILL.md body `When to Activate` and `Integration` sections must be audited the same day so the body does not contradict the description that routed the agent to it. The router benchmark sees descriptions only (`--ignore-rules`, no preloaded skills); Stage 3 effectiveness runs explicitly preload skill bodies and measure body-alignment impact.
...
- API keys provided in chat should be considered exposed and rotated immediately. The Codex runner does not read or fingerprint credentials; native Codex CLI owns OAuth state.
